...
import git_branch_cleaner/types.{
  type Branch, type BranchDiff, type GitError, Branch, BranchDiff,
}
import git_branch_cleaner/utils/listx
import gleam/list
import gleam/result

pub fn find_branches_to_cleanup() -> Result(List(Branch), GitError) {
...
fn diff_branches_against_ref(local_branches: List(Branch)) {
  ...
  ref_sub_branches
  |> list.map(git.diff_branches(starting_from: _, present_in: ref_branch))
  |> result.all()
}
